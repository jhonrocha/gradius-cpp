...
void Screen::setScreenWidth(int screen_width) {
  ...
  screen = SDL_SetVideoMode(screen_width, screenHeight, 32,
                            SDL_SWSURFACE | SDL_RESIZABLE);
}

void Screen::setScreenHeight(int screen_height) {
  this->screenHeight = screen_height;
  screen = SDL_SetVideoMode(screenWidth, screen_height, 32,
                            SDL_SWSURFACE | SDL_RESIZABLE);
}

int Screen::getScreenWidth() { return screenWidth; }

int Screen::getScreenHeight() { return screenHeight; }

void Screen::showScreen() { SDL_Flip(screen); }
