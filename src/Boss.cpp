...
Boss::Boss(int x, int y): Enemy("assets/Boss.png", x, y){
	life = 15;
	type = ENTITY_BOSS;
	moveType = rand()%300;
	moveDirection = 1;
...
void Boss::shoot(Timer globalTime, list<Entity*>* enemyList){
	if(lastFireTime + 1000 < globalTime.get_ticks()) {
		...
		Bullet* tiro = new Double("assets/BossShoot2.png", (this->x + 130), (this->y + 10), false);
		tiro->setXVel(12);
		enemyList->push_back(tiro);
		tiro->instanceCount();
		tiro->soundShoot();

		tiro = new Simple("assets/BossShoot1.png", (this->x + 95), (this->y + 55), false);
		tiro->setXVel(12);
		enemyList->push_back(tiro);
		tiro->instanceCount();
		tiro->soundShoot();

		tiro = new Simple("assets/BossShoot1.png", (this->x - 2), (this->y + 100), false);
		tiro->setXVel(12);
		enemyList->push_back(tiro);
		tiro->instanceCount();
		tiro->soundShoot();

		tiro = new Double("assets/BossShoot2.png", (this->x + 10), (this->y + 150), false);
		tiro->setXVel(12);
		enemyList->push_back(tiro);
		tiro->instanceCount();
		tiro->soundShoot();
