...
GradiusShip::GradiusShip(int x, int y) : Entity("assets/gradius.png", x, y) {
  life = 3;
  powerup = 0;
  upgrade = 0;
  shieldIsOn = false;
  ...
  type = ENTITY_GRADIUS;
  sound.loadFile("assets/sounds/GradiusExplosion.wav");
  shield = NULL;
}
...
int GradiusShip::getLife() { return life; }

void GradiusShip::setUpgrade(int value) { upgrade = value; }

int GradiusShip::getUpgrade() { return upgrade; }

void GradiusShip::setShield(bool observerShield) {
  this->observerShield = observerShield;
}

bool GradiusShip::shieldState() { return observerShield; }

Entity *GradiusShip::getShield() { return shield; }
...
void GradiusShip::move(int velX, int velY, int screenHeight, int screenWidth) {
  ...
  Box.y = y;
  if (shieldIsOn) {
    shield->move(this->x + this->width / 2 - shield->getWidth() / 2,
                 this->y + this->height / 2 - shield->getHeight() / 2);
  }
}
...
void GradiusShip::beDamaged(int damage) { this->life -= damage; }
