...
Controler::Controler()
    : FRAME_PER_SECOND(50), gradius(stage.screenHeight / 2, 300) {
  score = 0;
  quit = false;
  isShooting = false;
  isPaused = false;
...
void Controler::setBottomBar() {
  Sprite *sprite;
  sprite = new Sprite("assets/ALL_OFF.png", stage.screenWidth / 2 - 400,
                      stage.screenHeight - 40);
  bottomBar.push_back(sprite);
  sprite = new Sprite("assets/SPEED_ON.png", stage.screenWidth / 2 - 400,
                      stage.screenHeight - 40);
  bottomBar.push_back(sprite);
  sprite = new Sprite("assets/SHIELD_ON.png", stage.screenWidth / 2 - 400,
                      stage.screenHeight - 40);
  bottomBar.push_back(sprite);
  sprite = new Sprite("assets/SHOOT_ON.png", stage.screenWidth / 2 - 400,
                      stage.screenHeight - 40);
  bottomBar.push_back(sprite);
}
...
void Controler::StageLoop() {
  ...
  backgroundMusic->loadFile("assets/sounds/stage1.wav");
  backgroundMusic->playSound();
  timerID1 = SDL_AddTimer(3500, &Controler::FactoryEnemy, NULL);

  while (!quit && globalTime.get_ticks() - startTime <= 180000) {
    fps.start();
...
void Controler::BossTransition() {
  backgroundMusic = new Music();
  backgroundMusic->loadFile("assets/sounds/Warning.wav");
  backgroundMusic->playSound();
...
void Controler::BossLoop() {
  backgroundMusic = new Music();
  backgroundMusic->loadFile("assets/sounds/boss1.wav");
  backgroundMusic->playSound();
...
void Controler::BossDeathAnimation(Entity *boss) {
  ...
  quit = true;
  backgroundMusic = new Music();
  backgroundMusic->loadFile("assets/sounds/King_of_Kings.wav");
  backgroundMusic->playSound();
  WinScreen Win;
}
...
void Controler::ManageEntities() {
  ...
  for (it2 = enemyList.begin(); it2 != enemyList.end(); ++it2) {
      ...
      if (!gradiusList.empty() && !enemyList.empty()) {
        if ((*it2)->checkCollision(*it1)) {
          if ((*it2)->EntityType() != ENTITY_UPGRADE) {
            if ((*it1)->EntityType() != ENTITY_GRADIUS) {
              ...
              if (!(*it1)->isAlive()) {
                ...
                it2 = enemyList.end();
                backgroundMusic = new Music();
                backgroundMusic->loadFile("assets/sounds/Loser_of_Losers.wav");
                backgroundMusic->playSound();
                GameOver gameover;
                quit = true;
                return;
          ...
          if (it2 == enemyList.begin() || it2 == enemyList.end()) {
            ...
            } else if ((*it1)->EntityType() == ENTITY_GRADIUS &&
                       (*it2)->EntityType() == ENTITY_UPGRADE) {
              (*it1)->addUpgrade();
              delete (*it2);
              it2 = enemyList.erase(it2);
            }
    ...
    if (!enemyList.empty()) {
      ...
      if ((*it2)->endScreen(stage.screenWidth, stage.screenHeight) &&
          (*it2)->EntityType() != ENTITY_BOSS) {
        delete (*it2);
        it2 = enemyList.erase(it2);
        --it2;
      } else {
...
Uint32 Controler::FactoryEnemy(Uint32 interval, void *p) {
  ...
  if (sort <= 1) {
    timerID2 = SDL_AddTimer(250, &Controler::WaveEnemySimple,
                            (void *)(intptr_t)position);
  } else if (sort == 2) {
    timerID2 = SDL_AddTimer(350, &Controler::WaveEnemySquad1,
                            (void *)(intptr_t)position);
  } else {
    timerID2 = SDL_AddTimer(300, &Controler::WaveEnemySquad2,
                            (void *)(intptr_t)position);
  }
  ...
  if (myfile.is_open()) {
    ...
    for (int i = 1; i < sizeGlist; i++) {
      if (myfile.good()) {
        myfile >> x;
        myfile >> y;
        bullet = new Simple("assets/tiro1.png", x, y, true);
        bullet->Load(myfile);
