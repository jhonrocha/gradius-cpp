...
bool Button::handle_events(Event event) {
  ...
  if (event.EventType() == SDL_MOUSEBUTTONDOWN) {
    ...
    if (event.EventButton() == SDL_BUTTON_LEFT) {
      ...
      if ((mouseBox.x > buttonBox.x) &&
          (mouseBox.x < buttonBox.x + buttonBox.w)) {
        if ((mouseBox.y > buttonBox.y) &&
            (mouseBox.y < buttonBox.y + buttonBox.h)) {
...
bool Button::isClicked() { return click; }
